...
result: 证明了该算法能收敛至约1/sqrt(k)的近似纳什均衡，并实现了状态与动作空间样本复杂度的分离。
conclusion: 该研究为受限观测下的大规模多智能体协同学习提供了有效的理论框架和算法支持。
---

## 摘要
许多大规模平台和网络化控制系统都包含一个中心化决策者，在严格的可观测性限制下与海量智能体群体进行交互。受此类应用的启发，我们研究了在通信受限机制下，由一个全局智能体和 $n$ 个同质局部智能体组成的合作式马尔可夫博弈，其中全局智能体在每个时间步仅观测 $k$ 个局部智能体状态的子集。我们提出了一种交替学习框架 (\texttt{ALTERNATING-MARL})，其中全局智能体针对固定的局部策略执行子采样平均场 $Q$-学习，而局部智能体通过在诱导马尔可夫决策过程 (MDP) 中进行优化来更新。我们证明了这些近似最佳响应动力学收敛于 $\widetilde{O}(1/\sqrt{k})$-近似纳什均衡，同时在联合状态空间和动作空间的样本复杂度之间产生了分离。最后，我们在多机器人控制和联邦优化的数值模拟中验证了我们的结果。
