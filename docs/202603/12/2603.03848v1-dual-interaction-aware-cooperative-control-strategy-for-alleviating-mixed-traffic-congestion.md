...
score: 6.0
evidence: 用于交互感知的多智能体强化学习
tldr: 针对混合交通环境下人类驾驶车辆（HDV）行为的不确定性，本文提出了一种双重交互感知协作控制策略（DIACC）。该策略基于多智能体强化学习，通过区分CAV间的协作与CAV-HDV间的观测交互，增强了局部和全局的交互感知能力。结合软最小值聚合奖励设计和主动安全动作修正模块，DIACC在瓶颈路段显著提升了交通效率和系统适应性。
selection_source: fresh_fetch
motivation: 混合交通中HDV行为的多样性和不确定性给CAV的协作控制带来了巨大挑战。
method: 提出DIACC策略，包含局部交互自适应决策模块、全局交互增强评价器以及基于主动安全的动作修正模块。
result: 实验表明，DIACC在提升交通效率和适应性方面显著优于传统的规则模型和基准MARL模型。
conclusion: 通过增强双重交互感知，DIACC能有效缓解混合交通瓶颈区域的拥堵问题。
---

## 摘要
随着智能交通系统（ITS）的发展，联网自动驾驶车辆（CAVs）有望通过协同策略（例如在瓶颈区域）显著缓解交通拥堵。然而，混合交通环境中人类驾驶车辆（HDVs）行为的不确定性和多样性为 CAV 的协同带来了重大挑战。本文提出了一种双重交互感知协同控制（DIACC）策略，在多智能体强化学习（MARL）框架下，增强了混合交通瓶颈场景中联网自动驾驶车辆（CAVs）的局部和全局交互感知。DIACC 策略包含三项关键创新：1）去中心化交互自适应决策（D-IADM）模块，通过区分 CAV-CAV 协同交互与 CAV-HDV 观测交互，增强了 Actor 的局部交互感知；2）中心化交互增强 Critic（C-IEC），通过交互感知价值估计提升了 Critic 对全局交通状况的理解，为策略更新提供更准确的指导；3）一种奖励设计，采用带温度退火的 softmin 聚合，以优先处理混合交通中交互密集的场景。此外，轻量级的主动安全动作精炼（PSAR）模块应用基于规则的修正，以加速训练收敛。实验结果表明，与基于规则的模型和基准 MARL 模型相比，DIACC 显著提高了交通效率和适应性。
