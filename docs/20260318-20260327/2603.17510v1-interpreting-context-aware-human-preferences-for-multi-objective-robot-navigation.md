...
method: 结合视觉语言模型提取环境语义、大语言模型生成行为规则，并通过偏好转换模块驱动预训练的多目标强化学习策略。
result: 实验和用户研究证明，该系统能准确捕捉用户意图并生成一致的偏好向量，在多种室内场景中实现了灵活的行为自适应。
conclusion: 该框架通过整合高层语义推理与低层运动控制，显著提升了机器人在共享环境中导航的适应性、透明度和可用性。
---
