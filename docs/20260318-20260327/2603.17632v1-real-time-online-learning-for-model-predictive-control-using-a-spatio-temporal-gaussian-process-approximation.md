...
score: 6.0
evidence: 用于模型预测控制的精确状态轨迹预测
tldr: 本研究针对模型预测控制（MPC）中模型不准确的问题，提出了一种基于时空高斯过程（GP）近似的高效在线学习方法。传统GP-MPC在在线更新时计算负担沉重，难以满足实时性需求。该方法通过优化时空GP模型，实现了恒定计算复杂度的在线学习，能够实时捕捉系统动力学的变化。在自主微型赛车的仿真和硬件实验中，该方法显著提升了控制精度和对时变系统的适应能力。
selection_source: fresh_fetch
motivation: 解决高斯过程模型预测控制（GP-MPC）在线学习计算复杂度高、难以满足实时性要求的问题。
method: 提出一种高效的时空高斯过程近似模型，通过恒定的计算复杂度实现动态系统的在线学习。
result: 仿真和硬件实验（自主微型赛车）证明该方法能实时学习更准确的系统动力学，提升控制性能。
conclusion: 该方法为时变系统的实时在线学习控制提供了一种高效且鲁棒的解决方案。
---
